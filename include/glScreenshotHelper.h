...
#define DEFAULT_PATH "/media/uba" ///< Путь до каталога, где монтируются USB-накопители.
...
namespace vniiftri
{
    namespace oscilloscope_gui
    {
        ...
        {
            public:
                static std::vector<std::string> i_tals_checkFreeSpace(std::vector<std::string> catalogUsbNames, const char *screenshotType);
                static bool i_tals_isEnoughSpace(const char *pathToCatalog, const char *screenshotType);

                /**
                 * @brief Функция создает название скриншота
                 * 
                 * @details Название файла предстваляет из себя: 
                 * 
                 ...
                 * @return std::string - название файла скриншота
                 */
                 ...
                 * 
                 * @details В библиотеке существует макрос DEFAULT_PATH, в котором содержится путь до каталога, куда монтируются USB-накопители. 
                 * 
                 ...
                 * 
                 * @details Поочередно вызваются функции:
                 *  
                 * 1. Функция создания названия скриншота.
                 * 2. Функция поиска названия(ий) сомнтированного(ых) каталога(ов) USB-накопителя. 
                 * 
                 * С помощью функции glReadPixels берется массив пикселей из фреймбуффера и при помощи функции библиотеки FreeImage 
                 ...
                 * @code
                 ...
                 int main()
                 {
                     ScreenshotHelper::i_tals_takeAndLoadScreenshot("bmp", 1920, 1080);
                 }
                 * @endcode
                 *
                 * @param type тип файла скриншота (.png, .jpg, .bmp) 
