...
namespace vniiftri
{
    namespace oscilloscope_gui
    {
        ...
        void TestCreateFileName::test_i_tals_createFileName()
        {
            ...
            std::string fileName = std::string("screenshot_25-05-23_") + std::to_string(now->tm_hour) + ":" + std::to_string(now->tm_min) + ":" + std::to_string(now->tm_sec) + (".jpg");

            CPPUNIT_ASSERT_EQUAL(fileName, ScreenshotHelper::i_tals_createFileName("jpg"));

            sleep(delaySec);
            fileName = std::string("screenshot_25-05-23_") + std::to_string(now->tm_hour) + ":" + std::to_string(now->tm_min) + ":" + std::to_string(now->tm_sec + delaySec) + (".jpg");

            CPPUNIT_ASSERT_EQUAL(fileName, ScreenshotHelper::i_tals_createFileName("jpg"));

            /*   Сравнение названий файла без времени   */
            fileName = "screenshot_25-05-23.jpg";
